// This is a redifinition of the skillsharing_client component in this chapter
...
// The function renderTalk was rewritten to the class Talk
...
class Talk {
  ...
  syncState(talk) {
    this.talk = talk;
    // Deletes the comments that are still displayed
    this.comments.textContent = '';
    // Creates the updated comments
...
var SkillShareApp = class SkillShareApp {
  ...
  syncState(state) {
    ...
    for (let talk of state.talks) {
      ...
      // Talk already exists, comments will be updated
      if (
        cmp &&
        ...
        // Talk does not exist yet, new talk is being created
    ...
    // Talk has been deleted, component is removed
    for (let title of Object.keys(this.talkMap)) {
      // The title no longer exists in state.talks
